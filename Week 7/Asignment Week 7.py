...
@app.route("/api/users")
def api():
    mycursor.execute("select id, name,username from customers where username=%s",(request.args.get("username",""),))
    myresult=mycursor.fetchone()
    title=("id","name","username")
    data={"data":""}
    ...
    if myresult==None:
        ...
        data["data"]=dict(zip(title,myresult))
        return json.dumps(data)
